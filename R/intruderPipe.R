...
intruderPipe <- function(reads.filename = NA,
   ...
   if (!dir.exists(np_ref.dir)){dir.create(np_ref.dir)}
   ...
   if (nrow(tx.df) > 0 & (!file.exists(orf_csv_file) | overwrite_np)){
      ...
      intruderPep.out <- intruderPep(tx.df = tx.df,
                                     ...
                                     file.prefix = sampleIdRNA,
                                     blat.filter = FALSE)
      pep.rds_filename <- file.path(np.dir,'np.rds')
      saveRDS(file = pep.rds_filename,object = intruderPep.out)
      ...
      write.table(x =data.frame('np_output'=intruderPep.out[['tx_log']]), file =  tx_log_filename,quote = FALSE,row.names = FALSE,col.names = TRUE)
